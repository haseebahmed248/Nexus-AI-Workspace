...
export class WorkSpaceController {
  ...
  getSpecificWorkSpaces: RequestHandler = async (req, res) => {
    try {
      ...
      const data = await this.workSpaceService.findAssignedWorkSpaces(
        req.params.email
      );
      res.status(201).json({
        success: true,
  ...
  getWorkspaceUsers: RequestHandler = async (req, res) => {
    try {
      const members = await this.workSpaceService.getWorkspaceUsers(
